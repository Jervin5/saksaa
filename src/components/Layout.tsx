...
import { Search, User, ShoppingBag, Menu, X, ChevronDown, Phone, Mail, MapPin, Instagram, Facebook, Twitter, Bookmark, ShieldCheck } from 'lucide-react';
  ...
  return (
    <header className={cn(
      ...
      <div className="container mx-auto px-4 flex items-center justify-between">
        {/* Logo */}
        <Link to="/saksaa/" className="text-2xl font-serif font-bold tracking-widest text-brand-deep-green">
          SAKSAAS
        </Link>
                  ...
                  <div className="bg-white shadow-2xl rounded-lg p-8 w-[600px] grid grid-cols-3 gap-8 border border-stone-100">
                    <div>
                      ...
                      <ul className="space-y-2">
                        ...
                        <li><Link to="/shop?category=Necklace" className="text-sm hover:text-brand-deep-green">Necklace</Link></li>
                      </ul>
                    </div>
                    <div>
        ...
        <div className="flex items-center space-x-5">
          ...
          <Link to="/login" className="text-stone-700 hover:text-brand-deep-green transition-colors">
            <User size={20} />
          </Link>
          {user?.role === 'Admin' && (
            <Link to="/admin" className="text-brand-gold hover:text-brand-deep-green transition-colors">
              <ShieldCheck size={20} />
            </Link>
          )}
          <Link to="/wishlist" className="text-stone-700 hover:text-brand-deep-green transition-colors relative">
            <Bookmark size={20} />
            {wishlist.length > 0 && (
          ...
          <Link to="/cart" className="flex items-center space-x-2 text-stone-700 hover:text-brand-deep-green transition-colors group">
            <div className="relative">
              <ShoppingBag size={20} />
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 bg-brand-deep-green text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center">
                  {totalItems}
                </span>
              )}
            </div>
            <span className="hidden md:inline text-sm font-medium">₹{totalPrice.toLocaleString()}</span>
          </Link>
          <button 
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="space-y-6">
            <Link to="/saksaa/" className="text-2xl font-serif font-bold tracking-widest text-white">
              SAKSAAS
            </Link>
